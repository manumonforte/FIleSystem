...
DISK_LBA getBlockId(MyFileSystem *myFileSystem, NodeStruct *node, int logB)
{
    DISK_LBA block[BLOCK_SIZE_BYTES / sizeof(DISK_LBA)];
	DISK_LBA phyB;
	int m = BLOCK_SIZE_BYTES / sizeof(DISK_LBA);
	int k;

	if (logB > MAX_BLOCKS_PER_FILE ||
			logB >= NUM_DIRECT_LINKS + m)
		return -1;

	// Completar
	if(logB < NUM_DIRECT_LINKS){
		phyB = node->blocks[logB];
	}
	else if(logB < (NUM_DIRECT_LINKS+m)){
		if(readBlock(myFileSystem,node->simple,block)){
			return -1;
		}
		phyB = block[logB-NUM_DIRECT_LINKS];
	
	}
	return phyB;
}

DISK_LBA reserveBlock(MyFileSystem *myFileSystem)
{
	int i=0;
	//Recorremos mascara de bits
	while(i < NUM_BITS){
		if(myFileSystem->bitMap[i]==0){//si encontramos bloque libre
			myFileSystem->bitMap[i]==1;
			return i;
		}
		i++;
	}
	//si la i es igual NUM_BIT no lo hemos encontrado
	return -1;
}

int setBlockId(MyFileSystem *myFileSystem, NodeStruct *node, int logB, DISK_LBA phyB)
{
    DISK_LBA block[BLOCK_SIZE_BYTES / sizeof(DISK_LBA)];
	int m = BLOCK_SIZE_BYTES / sizeof(DISK_LBA);
	int k;
	...
	if (logB < NUM_DIRECT_LINKS) {
		...
		if (node->simple == -1) {
			/* El bloque de indices indirecto simple no está reservado
			 * hay que reservar un bloque, actualizar los campos del nodo-i
			 * relacionados e inicializar el bloque
			 */
			int b = reserveBlock(myFileSystem);
			// Completar
			node->simple=0;
			node->freeNode = 1;
...
void copyNode(NodeStruct *dest, NodeStruct *src)
{
    ...
    for(i = 0; i < NUM_DIRECT_LINKS ; i++)
		dest->blocks[i] = src->blocks[i];

	dest->simple = src->simple;
}
...
int reserveBlocksForNodes(MyFileSystem *myFileSystem, NodeStruct *node, int numBlocks)
{
    int i = 0;
    int currentBlock = 0;
    ...
    while((currentBlock < numBlocks) && (i < NUM_BITS)) {
        if(myFileSystem->bitMap[i] == 0) {
            myFileSystem->bitMap[i] = 1;
            setBlockId(myFileSystem, node, currentBlock, i);
            currentBlock++;
        }
        ++i;
    }
