...
typedef struct NodeStructure {
    ...
    DISK_LBA blocks[NUM_DIRECT_LINKS];			// Blocks
	DISK_LBA simple;
...
/**
 * @brief Get the DISK_LBA for a given logic block 
 *
 * @param myFileSystem the address of the filesystem structure
 * @param node the address of the node
 * @param logB the number of the logic block
 * @return DISK_LBA the physical block used to store the logic block, -1 in case
 * of error.
 **/
DISK_LBA getBlockId(MyFileSystem *myFileSystem, NodeStruct *node, int logB);

/**
 * @brief Finds a free block in the file system and set it as non free 
 *
 * @param myFileSystem the address of the filesystem structure
 * @return the block reserved
 **/
DISK_LBA reserveBlock(MyFileSystem *myFileSystem);

/**
 * @brief Sets the DISK_LBA (block id) used to store a give logical block.
 *
 * @param myFileSystem the address of the filesystem structure
 * @param inode the address of the i-node.
 * @param logB the number of the logic block
 * @param phyB the DISK_LBA (physical block id) used to strore the logic block
 * @return -1 in case of error, 0 otherwise.
 **/
int setBlockId(MyFileSystem *myFileSystem, NodeStruct *inode, int logB, DISK_LBA phyB);
 ...
 *
 * @param myFileSystem pointer to the FS
 * @param node the i-node of the file
