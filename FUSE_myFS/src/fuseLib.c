...
int resizeNode(uint64_t idxNode, size_t newSize)
{
    ...
    if(diff > 0) {
        ...
        if(node->numBlocks && node->fileSize % BLOCK_SIZE_BYTES) {
            int currentBlock = getBlockId(&myFileSystem, node, node->numBlocks-1);
		...
		int newIndexBlocks = 0;
        if(newBlocks) {
            memset(block, 0, sizeof(char)*BLOCK_SIZE_BYTES);

			//Check if we need extra blocks for index
			if (node->numBlocks + newBlocks > NUM_DIRECT_LINKS &&
					node->simple == -1) {
				newIndexBlocks++;
			}

            // We check that there is enough space
            if(newBlocks + newIndexBlocks > myFileSystem.superBlock.numOfFreeBlocks)
                return -ENOSPC;

            myFileSystem.superBlock.numOfFreeBlocks -= newBlocks + newIndexBlocks;
            ...
            for(i = 0; currentBlock != node->numBlocks; i++) {
                if(myFileSystem.bitMap[i] == 0) {
					...
					if (setBlockId(&myFileSystem, node, currentBlock, i) == -1)
						return -EIO;
                    currentBlock++;
    ...
    /// File decreases
        ...
        for(i = node->numBlocks; i > numBlocks; i--) {
            int nBloque = getBlockId(&myFileSystem, node, i - 1);
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
    ...
    myFileSystem.nodes[idxNodoI]->numIndexBlocks = 0;
	myFileSystem.nodes[idxNodoI]->simple = -1;
    myFileSystem.nodes[idxNodoI]->modificationTime = time(NULL);
    myFileSystem.nodes[idxNodoI]->freeNode = false;

    reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNodoI], 0);
	...
	struct fuse_file_info *fi )
{
    ...
    ///fi->fh, es el descriptor de fichero contenido en fuse_file_info
    NodeStruct *node = myFileSystem.nodes[fi->fh];
    ...
    while (totalRead < bytes2Read) {
    	...
    	//Obtenemos bloque actual dividieno el offset entre tamano de bloque
    	currentBlock = node->blocks[offset/BLOCK_SIZE_BYTES];

    	//Parte del offset que queda como resto de la division anterior
    	offBlock = offset%BLOCK_SIZE_BYTES;

    	/*Guardamos en el buffer el bloque en el que nos hemos posicionado aprovehcando
	 la funcion readBlock de myFS. buffer← bloque currentblock del disco virtual copiar
	 los bytes leıdos al buffer de salida buffer*/
    	int rdo = readBlock(&myFileSystem,currentBlock,&buffer);

	if (rdo !=-1){
		
		//Leemos el contenido desde el OffBlock hasta el sig bloque
		for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < bytes2Read); i++) {
            		array[totalRead++] = buffer[i];
        	}
    		//actualizar offset y totalRead
        	offset += (i - offBlock);
		//Actualizamos estructuras del SF
		node->modificationTime = time(NULL);
  		updateSuperBlock(&myFileSystem);
    		updateBitmap(&myFileSystem);
    		updateNode(&myFileSystem, fi->fh, node);
	}else{
		printf("Fallo en la lectura del Bloque\n");
		return -EIO;
...
static int my_unlink(const char *path)
{
	int idxNode;
	int indexOfFileDir;
	//Buscar path en el directorio del SF, aprovechamos la funcion de la libreria myFS.h
        //quitar el primer character (/) sumando uno al path
	indexOfFileDir = findFileByName(&myFileSystem, (char*)path+1); 
	if(indexOfFileDir !=-1){
		//Asignamos al nodo 
		...
		myFileSystem.numFreeNodes++;
		//Actualizar el directorio en el disco 
		updateDirectory(&myFileSystem);		
		...
		//Actualizamos los superbloques quitando el nodo que hemos eliminado
		//myFileSystem.directory.files[idxNode].free = true;
		...
		//Parcheamos unlink
		int x =myFileSystem.nodes[idxNode]->simple;
		if(x!=-1){
			//marcamos como libre el bloque en el mapa de bits
			myFileSystem.bitMap[x]=0;
		}
		updateSuperBlock(&myFileSystem);
		updateBitmap(&myFileSystem);
...
struct fuse_operations myFS_operations = {
    ...
    .read 	=my_read,					// Read data from files -->Implementado
    .unlink     =my_unlink,					// Delete a file -->Implementado
};
